...
<html>
<head>
	<title>BOOKWORM||<?php if(isset($_GET['genre'])){echo ucwords($genre);} else{echo ucwords($tag);} ?></title>
...
<body>
    ...
    <div class="bookdetails1 container-fluid pl-4 pt-4 pb-4">
    	<h2 class=" text-center mb-5"><?php if(isset($_GET['genre'])){echo ucwords($genre);} else{echo ucwords($tag);} ?></h2>
    	<div class="row">
    		...
    		<div class="col-sm-10 row pl-4">
    			<?php
    				...
    				if(isset($_GET['genre'])){
	    				...
	    				$stmt = $conn->prepare("SELECT book.pic, book.title, book.book_isbn as isbn, min( book_seller.price) as price, GROUP_CONCAT(DISTINCT author.name) as author, GROUP_CONCAT(distinct user.name) as seller, GROUP_CONCAT(DISTINCT tag.name) as tag from book_author join book ON book_author.book_id = book.id
		                    JOIN author on book_author.author_id = author.id
		                    JOIN book_seller on book_seller.book_id = book.id
		                    JOIN `user` on user.id = book_seller.user_id
                    ...
                    if($result->num_rows > 0){
                    	while ($row = $result->fetch_assoc()) {
                    		...
                    		"<div class='col-sm-3 pl-1 pr-1 mb-2 mt-2 text-center pt-2 pb-2'>
                    			...
                    			<div class='bg-success rounded mt-1 pt-1 pb-1 font-weight-bold'><a href='book.php?isbn=".$row['isbn']."' style='text-decoration: none' class='text-light'>".ucwords($row['title'])."</a></div>
                    			...
                    			<div class='bg-light rounded font-weight-bold h5 text-success mt-1' style='margin-top:0'><span>&#8377</span>".$row['price']."</div>
                    		</div>";
                    	}
                    }
    			?>
