<?php
	...
	if ($_SESSION['user_type_id'] != 2) {
		...
		if($result->num_rows != 0){
			...
			if($row['if_wishlist'] == 1){
				$stmt = $conn->prepare("UPDATE cart SET if_wishlist = 0, updated_at = ? WHERE user_id=? and book_seller_id=? and order_uid IS NULL and if_wishlist = 0");
